...
router.post("/upvotes/:id", fetchUser, async (req, res) => {
    try {
        ...
        const upvoteIndex = post.upvotes.findIndex((id) => isObjectIdEqual(id, userId));
        const downvoteIndex = post.downvotes.findIndex((id) => isObjectIdEqual(id, userId));
...
router.post("/reactions/toggle", fetchUser, async (req, res) => {
    try {
        ...
        if (existingReaction && existingReaction.type === type) {
            ...
            targetDoc.reactionCounts[type] = Math.max(0, (targetDoc.reactionCounts[type] || 0) - 1);
